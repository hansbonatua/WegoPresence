...
import { approve, cancel, create, index, reject } from '@/routes/business-trips';
import type {
    BusinessTripCan,
    BusinessTripFilters,
...
type IndexProps = {
    businessTrips: Paginated<BusinessTripResource>;
    filters: BusinessTripFilters;
    can: BusinessTripCan;
};

const statusStyles: Record<BusinessTripStatus, string> = {
    pending:
...
export default function BusinessTripsIndex({
    businessTrips,
    filters,
    can,
}: IndexProps) {
    ...
    const [status, setStatus] = useState<BusinessTripStatus | ''>(
        filters.status,
    );
    const [isLoading, setIsLoading] = useState(false);
    const [selected, setSelected] = useState<BusinessTripResource | null>(
        null,
    );
    const [toCancel, setToCancel] = useState<BusinessTripResource | null>(
        null,
    );
    const [toReview, setToReview] = useState<{
        businessTrip: BusinessTripResource;
    ...
    function openReview(
        ...
        setApprovalNotes('');
        setToReview({ businessTrip, action });
    }
    ...
    function confirmReview() {
        ...
        const route =
            toReview.action === 'approve'
                ? approve({ business_trip: toReview.businessTrip.id })
                : reject({ business_trip: toReview.businessTrip.id });
    ...
    function confirmCancel() {
        ...
        router.post(cancel({ business_trip: toCancel.id }), undefined, {
            preserveScroll: true,
            onFinish: () => {
                setIsSubmitting(false);
                setToCancel(null);
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-5 bg-sky-400 p-4 rounded-lg text-white sm:flex-row sm:items-center sm:justify-between">
                    <Heading
                        ...
                        description={
                            can.manage
                                ? 'Review and process business trip requests'
                                : 'Submit and track your business trip requests'
                        }
                    />
                ...
                <Card>
                    <CardHeader className="gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <CardTitle>Business trip requests</CardTitle>
                            <CardDescription>
                                {businessTrips.total > 0
                                    ? `${businessTrips.total} request${businessTrips.total === 1 ? '' : 's'} found`
                        ...
                        <div className="flex w-full flex-col gap-2 sm:max-w-md sm:flex-row">
                            ...
                            <div className="relative w-full">
                                ...
                                <Input
                                    ...
                                    placeholder="Search destination, purpose, name, or NIP..."
                                    className="pl-9"
                                    aria-label="Search business trip requests"
                                />
                            </div>
                        </div>
                    </CardHeader>
                    ...
                    <CardContent className="p-0">
                                                ...
                                                <div className="flex flex-col items-center gap-2 px-4 py-16 text-center">
                                                    ...
                                                    <p className="text-sm text-muted-foreground">
                                                        ...
                                                        filters.status
                                                            ...
                                                            : can.manage
                                                              ? 'Business trip requests will appear here once employees submit them.'
                                                              : 'Submit your first business trip request to get started.'}
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                        businessTrips.data.map((trip) => {
                                            const canCancel =
                                                trip.user_id ===
                                                    currentUserId &&
                                                trip.status === 'pending';
                        ...
                        {businessTrips.data.length > 0 && (
                            <div className="border-t p-4">
                                <Pagination links={businessTrips.links} />
                            </div>
                        )}
                    </CardContent>
                </Card>
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Business trip request</DialogTitle>
                        <DialogDescription>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {toReview && (
                                <>
                                    {toReview.businessTrip.user?.name} ·{' '}
                                    {toReview.businessTrip.start_date}
                                    {toReview.businessTrip.end_date !==
                                    toReview.businessTrip.start_date
                                        ? ` → ${toReview.businessTrip.end_date}`
                                        : ''}
                                </>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {toCancel && (
                                <>
                                    This will cancel the business trip request
